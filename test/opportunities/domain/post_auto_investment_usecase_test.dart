...
void main() {
  late PostAutoInvestmentDomainUseCase useCase;
  late MockOpportunitiesRepository mockRepo;

  setUp(() {
    mockRepo = MockOpportunitiesRepository();
    useCase = PostAutoInvestmentDomainUseCase(mockRepo);
  });

  final params = PostAutoInvestmentParams({"amount": 1000});

  test('should return Right(SuccessNewModel) when success', () async {
    final model = SuccessNewModel();

    when(() => mockRepo.postAutoInvestmentDomain(params.payload))
        .thenAnswer((_) async => Right(model));

    final result = await useCase(params);

    expect(result, Right(model));
    verify(() => mockRepo.postAutoInvestmentDomain(params.payload))
        .called(1);
  });
